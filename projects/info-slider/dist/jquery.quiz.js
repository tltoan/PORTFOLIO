...
(function ($) {
  ...
  $.quiz = $.fn.quiz = function (options) {
    ...
    function init() {
      ...
      $.getJSON(settings.quizJson + "?_=" + new Date().getTime())
        .done(function (data, textStatus, jqXHR) {
          ...
          if (
            typeof settings.numQuestions == "number" &&
            settings.numQuestions < quizArr.length
          ) {
    ...
    function htmlStructure() {
      ...
      var $quiz = $(DIV).prop("id", QUIZ_ID).appendTo(CONTAINER);

      // Append body area to quiz
      $(DIV).prop("id", BODY_ID).appendTo($quiz);

      // Append buttons area to quiz body
      $(DIV).prop("id", BUTTONS_ID).attr("role", "group").appendTo($quiz);

      // Append progress area to quiz
      $(DIV).prop("id", PROGRESS_ID).appendTo($quiz);
    }
    ...
    function htmlFirst() {
      ...
      if (intro) {
        ...
        $(TemplateEngine(settings.introTpl, intro)).appendTo("#" + BODY_ID);
    ...
    function htmlQuestion(q) {
      ...
      $question = $(
        TemplateEngine(settings.questionTpl, {
          question: question,
          answers: answers,
        })
      ).appendTo("#" + BODY_ID);
    ...
    function htmlResults() {
      ...
      $.each(quizArr, function (i, q) {
        ...
        // Append question to quiz body area
        $question = $(
          TemplateEngine(settings.resultsTpl, {
            question: question,
            answer: answer || false,
          })
        ).appendTo("#" + BODY_ID);
    ...
    function showProgress(id) {
      ...
      if (typeof settings.progressTpl == "string") {
        ...
        $("#" + PROGRESS_ID).html(
          TemplateEngine(settings.progressTpl, {
            progress: progress,
          })
        );
      }
    ...
    function showModal(error, back) {
      ...
      if (!$mdl.length) {
        ...
        var html =
          '<div id="' +
          MODAL_ID +
          '" class="modal fade" role="dialog" aria-hidden="true" aria-labelledby="quiz_mdltitle" tabindex="-1">' +
